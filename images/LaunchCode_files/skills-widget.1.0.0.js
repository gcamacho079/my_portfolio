...
Elm.Styles.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var colors = {buttonBgUnselected: "white",buttonBgSelected: "#5c93ce",buttonText: "#344a5f",buttonTextSelected: "white",titleColor: "#aaa"};
   var ColorSet = F5(function (a,b,c,d,e) {    return {buttonBgUnselected: a,buttonBgSelected: b,buttonText: c,buttonTextSelected: d,titleColor: e};});
   var checkbox = _U.list([{ctor: "_Tuple2",_0: "margin-right",_1: "4px"}]);
   var selectableOn = _U.list([{ctor: "_Tuple2",_0: "background-color",_1: colors.buttonBgSelected}
                              ,{ctor: "_Tuple2",_0: "border-width",_1: "0"}
                              ,{ctor: "_Tuple2",_0: "border",_1: "solid 1px #d8d9d9"}
                              ,{ctor: "_Tuple2",_0: "color",_1: colors.buttonTextSelected}]);
   var selectable = function (isSelected) {
      ...
      _U.list([{ctor: "_Tuple2",_0: "display",_1: "inline-block"}
              ...
              ,{ctor: "_Tuple2",_0: "background-color",_1: colors.buttonBgUnselected}]),
      isSelected ? selectableOn : _U.list([]));
   };
   var multiSelect = _U.list([{ctor: "_Tuple2",_0: "min-height",_1: "50px"},{ctor: "_Tuple2",_0: "margin-bottom",_1: "40px"}]);
   var multiSelectHeading = _U.list([{ctor: "_Tuple2",_0: "line-height",_1: "1.75"},{ctor: "_Tuple2",_0: "margin-bottom",_1: "20px"}]);
...
Elm.Selectable.make = function (_elm) {
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([]));
      return A2($Html.div,
      _U.list([A2($Html$Events.onClick,onClickAddress,NoOp)
              ,$Html$Attributes.style($Styles.selectable(model.isSelected))
              ,$Html$Attributes.$class("selectable-element")]),
...
Elm.Model.make = function (_elm) {
   ...
   var TextHeaders = F3(function (a,b,c) {    return {positionCategoryHeader: a,coreCompetencyHeader: b,skillHeader: c};});
   var Output = F3(function (a,b,c) {    return {positionCategoryIds: a,coreCompetencyIds: b,skillIds: c};});
...
Elm.JsonParser.make = function (_elm) {
   ...
   var posCatDecoder = A4($Json$Decode.object3,
...
Elm.MultiSelect.make = function (_elm) {
   ...
   $Styles = Elm.Styles.make(_elm);
   var _op = {};
   var update = F3(function (action,isMultiple,model) {
      var _p0 = action;
      var _p2 = _p0._1;
      var clearSelected = function (sel) {    return sel.isSelected ? A2($Selectable.update,_p2,sel) : sel;};
      var updateItem = function (sel) {    return _U.eq(sel.id,_p0._0) ? A2($Selectable.update,_p2,sel) : sel;};
      return isMultiple ? A2($List.map,updateItem,model) : A2($List.map,function (_p1) {    return updateItem(clearSelected(_p1));},model);
   });
...
Elm.SkillsWidget.make = function (_elm) {
   ...
   var availableSkills = function (model) {
      var idsFromAllCoreComps = $List$Extra.dropDuplicates(A2($List.concatMap,$Model.skillDependencies,model.coreCompetencies));
      var isMemberOfAnyCoreComp = function (id) {    return A2($List.member,id,idsFromAllCoreComps);};
      ...
      var isMemberOfSelectedPosCats = function (id) {    return A2($List.member,id,idsFromPosCats);};
      var idsFromCoreComps = availableIDs(model.coreCompetencies);
      var isMemberOfBoth = function (id) {    return isMemberOfSelectedPosCats(id) && A2($List.member,id,idsFromCoreComps);};
      return A2($List.filter,
      function (s) {
         var skillId = function (_p1) {    return function (_) {    return _.id;}(function (_) {    return _.selectable;}(_p1));}(s);
         return isMemberOfBoth(skillId) || isMemberOfSelectedPosCats(skillId) && $Basics.not(isMemberOfAnyCoreComp(skillId));
      },
      model.skills);
   };
   ...
   var multiSelectView = F3(function (msAddress,msModel,msName) {
      return $List.isEmpty(msModel) ? A2($Html.div,_U.list([]),_U.list([])) : A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.h3,_U.list([$Html$Attributes.style($Styles.multiSelectHeading)]),_U.list([$Html.text(msName)]))
   ...
   var outputModel = function (model) {
      var collectIds = $List.map(function (_p2) {    return function (_) {    return _.id;}(function (_) {    return _.selectable;}(_p2));});
      var filterIsOn = $List.filter(function (_p3) {    return function (_) {    return _.isSelected;}(function (_) {    return _.selectable;}(_p3));});
      var selectedPosCatIds = collectIds(filterIsOn(model.positionCategories));
      var selectedCoreCompIds = collectIds(filterIsOn(availableCompetencies(model)));
      var selectedSkillIds = collectIds(filterIsOn(availableSkills(model)));
      return {positionCategoryIds: selectedPosCatIds,coreCompetencyIds: selectedCoreCompIds,skillIds: selectedSkillIds};
   };
   var selected = Elm.Native.Port.make(_elm).inbound("selected",
   "Model.Output",
   function (v) {
      return typeof v === "object" && "positionCategoryIds" in v && "coreCompetencyIds" in v && "skillIds" in v ? {_: {}
                                                                                                                  ,positionCategoryIds: typeof v.positionCategoryIds === "object" && v.positionCategoryIds instanceof Array ? Elm.Native.List.make(_elm).fromArray(v.positionCategoryIds.map(function (v) {
                                                                                                                     return typeof v === "number" && isFinite(v) && Math.floor(v) === v ? v : _U.badPort("an integer",
                                                                                                                     v);
                                                                                                                  })) : _U.badPort("an array",
                                                                                                                  v.positionCategoryIds)
                                                                                                                  ,coreCompetencyIds: typeof v.coreCompetencyIds === "object" && v.coreCompetencyIds instanceof Array ? Elm.Native.List.make(_elm).fromArray(v.coreCompetencyIds.map(function (v) {
                                                                                                                     return typeof v === "number" && isFinite(v) && Math.floor(v) === v ? v : _U.badPort("an integer",
                                                                                                                     v);
                                                                                                                  })) : _U.badPort("an array",
                                                                                                                  v.coreCompetencyIds)
                                                                                                                  ,skillIds: typeof v.skillIds === "object" && v.skillIds instanceof Array ? Elm.Native.List.make(_elm).fromArray(v.skillIds.map(function (v) {
                                                                                                                     return typeof v === "number" && isFinite(v) && Math.floor(v) === v ? v : _U.badPort("an integer",
                                                                                                                     v);
                                                                                                                  })) : _U.badPort("an array",
                                                                                                                  v.skillIds)} : _U.badPort("an object with fields `positionCategoryIds`, `coreCompetencyIds`, `skillIds`",
      v);
   });
   var setInitialSelected = function (model) {
      var turnOnNestedSel = function (linkedSel) {
         var sel = linkedSel.selectable;
         var updatedSel = _U.update(sel,{isSelected: true});
         return _U.update(linkedSel,{selectable: updatedSel});
      };
      var turnOn = F2(function (selectedIds,linkedSelectable) {
         return A2($List.member,linkedSelectable.selectable.id,selectedIds) ? turnOnNestedSel(linkedSelectable) : linkedSelectable;
      });
      return _U.update(model,
      {positionCategories: A2($List.map,turnOn(selected.positionCategoryIds),model.positionCategories)
      ,coreCompetencies: A2($List.map,turnOn(selected.coreCompetencyIds),model.coreCompetencies)
      ,skills: A2($List.map,turnOn(selected.skillIds),model.skills)});
   };
   var textHeaders = Elm.Native.Port.make(_elm).inbound("textHeaders",
   "Model.TextHeaders",
   function (v) {
      return typeof v === "object" && "positionCategoryHeader" in v && "coreCompetencyHeader" in v && "skillHeader" in v ? {_: {}
                                                                                                                           ,positionCategoryHeader: typeof v.positionCategoryHeader === "string" || typeof v.positionCategoryHeader === "object" && v.positionCategoryHeader instanceof String ? v.positionCategoryHeader : _U.badPort("a string",
                                                                                                                           v.positionCategoryHeader)
                                                                                                                           ,coreCompetencyHeader: typeof v.coreCompetencyHeader === "string" || typeof v.coreCompetencyHeader === "object" && v.coreCompetencyHeader instanceof String ? v.coreCompetencyHeader : _U.badPort("a string",
                                                                                                                           v.coreCompetencyHeader)
                                                                                                                           ,skillHeader: typeof v.skillHeader === "string" || typeof v.skillHeader === "object" && v.skillHeader instanceof String ? v.skillHeader : _U.badPort("a string",
                                                                                                                           v.skillHeader)} : _U.badPort("an object with fields `positionCategoryHeader`, `coreCompetencyHeader`, `skillHeader`",
      v);
   });
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.style($Styles.skillsWidget)]),
      _U.list([A3(multiSelectView,A2($Signal.forwardTo,address,PosCats),extractSelectables(model.positionCategories),textHeaders.positionCategoryHeader)
              ,A3(multiSelectView,A2($Signal.forwardTo,address,CoreComps),extractSelectables(availableCompetencies(model)),textHeaders.coreCompetencyHeader)
              ,A3(multiSelectView,
              A2($Signal.forwardTo,address,Skills),
              A2($List.sortBy,function (_) {    return _.name;},extractSelectables(availableSkills(model))),
              textHeaders.skillHeader)]));
   });
   var isMultiple = Elm.Native.Port.make(_elm).inbound("isMultiple",
   "Bool",
   function (v) {
      return typeof v === "boolean" ? v : _U.badPort("a boolean (true or false)",v);
   });
   var updateLinkedSels = F2(function (msAction,linkedSels) {
      return A3($Basics.flip,mergeNewSelectables,linkedSels,A3($MultiSelect.update,msAction,isMultiple,extractSelectables(linkedSels)));
   });
   var update = F2(function (action,model) {
      return noEffects(function () {
         var _p4 = action;
         switch (_p4.ctor)
         {case "PosCats": return _U.update(model,{positionCategories: A2(updateLinkedSels,_p4._0,model.positionCategories)});
            case "CoreComps": return _U.update(model,{coreCompetencies: A2(updateLinkedSels,_p4._0,model.coreCompetencies)});
            default: return _U.update(model,{skills: A2(updateLinkedSels,_p4._0,model.skills)});}
      }());
   });
   ...
   var app = $StartApp.start({init: noEffects(setInitialSelected($JsonParser.parseJson(jsonData))),update: update,view: view,inputs: _U.list([])});
   var output = Elm.Native.Port.make(_elm).outboundSignal("output",
   function (v) {
      return {positionCategoryIds: Elm.Native.List.make(_elm).toArray(v.positionCategoryIds).map(function (v) {    return v;})
             ,coreCompetencyIds: Elm.Native.List.make(_elm).toArray(v.coreCompetencyIds).map(function (v) {    return v;})
             ,skillIds: Elm.Native.List.make(_elm).toArray(v.skillIds).map(function (v) {    return v;})};
   },
   A2($Signal.map,outputModel,app.model));
   var main = app.html;
